...
class Cifar100(CIFAR100):
    def __init__(self, root, train, unlearning, download, img_size=32, 
        ...
        # identity_range is unused for CIFAR but accepted for API consistency with MUCAC
...
class MUCAC(Dataset):
    """
    MUCAC Dataset with FIXED split logic and decoupled transform/identity selection.
    
    Identity ranges:
    - Test: identities 0-189
    - Train/Retain: identities 190-1969
    - Forget: identities 1970-4854
    
    FIXED: The `unlearning` parameter now ONLY controls transforms (no augmentation when True).
    Identity range selection is controlled by `identity_range` parameter, or defaults to
    backward-compatible train/unlearning logic when not specified.
    """
    def __init__(self, root, train, unlearning, download=False, img_size=128, 
                 indices=None, use_augmentation=True, identity_range=None):
        """
        Args:
            root: Root directory containing CelebAMask-HQ
            train: If True, load training samples; if False, load test samples
            unlearning: If True, use simple transforms (no augmentation) - ONLY affects transforms
            download: Unused (kept for API compatibility)
            img_size: Target image size (default 128)
            indices: Optional indices to select a subset of samples
            use_augmentation: If False, disable augmentation regardless of other flags
            identity_range: Override identity selection. One of:
                - None (default): Use train/unlearning logic for backward compatibility
        ...
        # Transforms - unlearning flag ONLY controls transforms, not identity range
        self.transform = get_mucac_transform(train, unlearning, use_augmentation)

        # Determine which identity range to load
        # FIXED: identity_range parameter allows decoupling from unlearning flag
        if identity_range is not None:
            # Explicit override for evaluation loaders
        ...
        for img_path in sorted(glob.glob(os.path.join(self.img_dir, "*.jpg"))):
            ...
            if load_forget and self.train_max_identity <= identity < self.forget_max_identity:
                ...
                self.image_paths.append(img_path)
                self.labels.append(smiling)
            elif load_test and identity < self.test_max_identity:
                self.image_paths.append(img_path)
                self.labels.append(smiling)
